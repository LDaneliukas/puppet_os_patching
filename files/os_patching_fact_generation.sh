...
case $(facter osfamily) in
  RedHat)
    ...
    PKGS=$(yum -q check-update 2>/dev/null| $FILTER | egrep -v "is broken|^Loaded plugins" | awk '/^[[:alnum:]]/ {print $1}' | cut -d. -f1)
    SECPKGS=$(yum -q --security check-update 2>/dev/null| $FILTER | egrep -v "is broken|^Loaded plugins" | awk '/^[[:alnum:]]/ {print $1}' | cut -d. -f1)
    HELDPKGS=$(awk '/^[[:alnum:]]/ {print $1}' /etc/yum/pluginconf.d/versionlock.list | sed -e 's/-[0-9].*//' -e 's/^[0-9]://')
  ;;
  Suse)
